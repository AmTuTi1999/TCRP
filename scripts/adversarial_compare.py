...
try:
    from tcrp.diagnostics.overfit import (
        bypass_ratio,  # type: ignore
        concept_direction_gap,
    )
...
def make_config(args: argparse.Namespace, adversarial: bool) -> TCRPConfig:
    """Build a TCRPConfig from parsed CLI arguments."""
    return TCRPConfig(
        H=args.H,
        d=64,
        K=0,
        ...
        encoder_in_ch=1,
        encoder_hidden=64,
        tcn_encoder_n_layers=4,
        tcn_encoder_kernel_size=3,
        tcn_encoder_use_weight_norm=True,
        lstm_encoder_bidirectional=False,
...
def run_experiment(args: argparse.Namespace) -> dict:
    """Train standard and adversarial TCRP models and collect comparison metrics."""
    ...
    with torch.no_grad():
        for x, y in val_loader:
            ...
            out = model_std(x)
            val_mse_std += torch.nn.functional.mse_loss(
                out.y_hat, y, reduction="sum"
            ).item()
            val_mae_std += torch.nn.functional.l1_loss(
                out.y_hat, y, reduction="sum"
            ).item()
            n += x.shape[0]
    val_mse_std /= n
    ...
    with torch.no_grad():
        for x, y in val_loader:
            x, y = x.to(device), y.to(device)
            out, _ = model_adv(x)
            val_mse_adv += torch.nn.functional.mse_loss(
                out.y_hat, y, reduction="sum"
            ).item()
            val_mae_adv += torch.nn.functional.l1_loss(
                out.y_hat, y, reduction="sum"
            ).item()
            n += x.shape[0]
    val_mse_adv /= n
    ...
    if _HAS_OVERFIT_DIAG:
        results["adversarial"]["bypass_ratio"] = bypass_ratio(
            model_adv, val_loader, device
        )
        results["adversarial"]["concept_dir_gap"] = concept_direction_gap(
            model_adv, train_loader, val_loader, device
        )
        results["standard"]["bypass_ratio"] = bypass_ratio(
            model_std, val_loader, device
        )
        results["standard"]["concept_dir_gap"] = concept_direction_gap(
...
def print_report(results: dict) -> None:
    """Print a formatted comparison report for standard vs adversarial TCRP."""
    print("\n" + "=" * 60)
    ...
    for name, r in results.items():
        ...
        if "mean_purity" in r:
            ...
            print(
                f"    Pure concepts      : {r['pure_concepts']}/{r['total_concepts']}"
            )
    ...
    if not mse_ok and purity_ok:
        mse_delta = (adv.get("val_mse", 0) - std.get("val_mse", 0)) / max(
            std.get("val_mse", 1), 1e-9
        )
        print(
...
def main() -> None:
    """Parse CLI arguments, run the comparison experiment, and write results."""
    parser = argparse.ArgumentParser(
        description="Adversarial vs standard TCRP comparison"
    )
