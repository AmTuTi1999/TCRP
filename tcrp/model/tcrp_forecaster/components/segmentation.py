...
class Segmenter(nn.Module):
    ...
    def __init__(self, L: int, stride: int):
        ...
        self._start_indices: Optional[Tensor] = None
        self._overlap_counts: Optional[Tensor] = None
        self._T_cached: Optional[int] = None
    
    @property
    def start_indices(self) -> Optional[Tensor]:
        """
    ...
    @property
    def overlap_counts(self) -> Optional[Tensor]:
        """
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if x.dim() != 2:
            raise ValueError(
                f"Expected input shape (B, T), got {tuple(x.shape)}"
            )
    ...
    def get_segment_info(self) -> dict:
        ...
        return {
            'L': self.L,
            'stride': self.stride,
            'N': self._start_indices.shape[0] if self._start_indices is not None else None,
            'T': self._T_cached,
