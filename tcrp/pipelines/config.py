...
class PipelineConfig:
    ...
    baseline_layers: int = 3                       # depth (lstm layers / tcn blocks / nbeats blocks-per-stack)
...
def tcrp_config_from_hydra(cfg: "DictConfig") -> "TCRPConfig":  # type: ignore[name-defined]
    ...
    m = cfg.models
    return TCRPConfig(
        H=int(cfg.H), L=int(m.L), stride=int(m.stride),
        d=int(m.encoder_hidden),
        K=0,
        periods=list(cfg.datasets.periods), k_max=int(m.k_max),
        alpha=float(m.alpha), beta=float(m.beta),
        gamma=float(m.gamma), gamma_j=float(m.gamma_j),
        jump_threshold=float(m.jump_threshold), train_std=float(m.train_std),
        lambda1=float(m.lambda1), lambda2=float(m.lambda2), lambda3=float(m.lambda3),
        probabilistic=bool(m.probabilistic),
        adversarial=bool(m.adversarial), alpha_max=float(m.alpha_max),
        warmup_epochs=int(m.warmup_epochs),
        encoder_in_ch=int(m.encoder_in_ch), encoder_hidden=int(m.encoder_hidden),
